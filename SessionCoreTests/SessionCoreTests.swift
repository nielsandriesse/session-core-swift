...
class SessionCoreTests : XCTestCase {
    ...
    func testGetSwarm() {
        let semaphore = DispatchSemaphore(value: 0)
        var error: Error? = nil
        let _ = SnodeAPI.getSwarm(for: mockSessionID).retryingIfNeeded(maxRetryCount: maxRetryCount).done(on: SnodeAPI.workQueue) { _ in
            semaphore.signal()
        }.catch(on: SnodeAPI.workQueue) {
            error = $0; semaphore.signal()
        }
    ...
    func testGetTargetSnodes() {
        let semaphore = DispatchSemaphore(value: 0)
        var error: Error? = nil
        let _ = SnodeAPI.getTargetSnodes(for: mockSessionID).retryingIfNeeded(maxRetryCount: maxRetryCount).done(on: SnodeAPI.workQueue) { _ in
            semaphore.signal()
        }.catch(on: SnodeAPI.workQueue) {
            error = $0; semaphore.signal()
        }
    ...
    func testGetMessages() {
        let semaphore = DispatchSemaphore(value: 0)
        var error: Error? = nil
        let _ = SnodeAPI.getMessages(for: mockSessionID).retryingIfNeeded(maxRetryCount: maxRetryCount).done(on: SnodeAPI.workQueue) { _ in
            semaphore.signal()
        }.catch(on: SnodeAPI.workQueue) {
            error = $0; semaphore.signal()
        }
    ...
    func testSendMessage() {
        let semaphore = DispatchSemaphore(value: 0)
        var error: Error? = nil
        let message = Message(destination: mockSessionID, data: mockData, ttl: mockTTL)
    ...
    func testCalculatePoW() {
        let _ = Message.calculatePow(ttl: mockTTL, destination: mockSessionID, data: mockData)
    }
}
